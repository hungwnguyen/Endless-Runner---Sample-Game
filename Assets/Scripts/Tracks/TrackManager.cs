...
public class TrackManager : MonoBehaviour
{
    ...
    public void StartMove(bool isRestart = true)
    {
        ...
        m_IsMoving = true;
        if (isRestart)
            m_Speed = Mathf.Max(0.1f, BienBaiHoc.tocDoChay);
    }
    ...
    public IEnumerator Begin()
    {
        if (!m_Rerun)
        {
            ...
            characterController.CheatInvincible(invincible || BienBaiHoc.batTu);
    ...
    void Update()
    {
        ...
        m_Multiplier = Mathf.Max(1, 1 + Mathf.FloorToInt((m_Speed - minSpeed) / (maxSpeed - minSpeed) * speedStep));
...
//"level" in our game are milestone the player have to reach : one every 300m
    ...
    {
        int finalAmount = amount + Mathf.Max(0, BienBaiHoc.diemThuongThem);
        m_Score += finalAmount * m_Multiplier;
    }

    void ApplyLessonVariables()
    {
    ...
    void OnGUI()
    {
        if (!BienBaiHoc.hienBangBien)
            return;

        Rect box = new Rect(10, 10, 330, 130);
        GUI.Box(box, "Bai hoc bien static");
        GUI.Label(new Rect(25, 35, 300, 20), "string tenNguoiChoi = " + BienBaiHoc.tenNguoiChoi);
        GUI.Label(new Rect(25, 55, 300, 20), "bool batTu = " + BienBaiHoc.batTu);
        GUI.Label(new Rect(25, 75, 300, 20), "float tocDoChay = " + BienBaiHoc.tocDoChay);
        GUI.Label(new Rect(25, 95, 300, 20), "int diemThuongThem = " + BienBaiHoc.diemThuongThem);
        GUI.Label(new Rect(25, 115, 300, 20), "Diem hien tai = " + m_Score);
    }
}
